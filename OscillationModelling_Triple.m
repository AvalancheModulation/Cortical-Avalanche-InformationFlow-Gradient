%To change latttice 1, 2 and 3 between periodic and aperiodic oscillator,interchange between mod(a,1)==0 [periodic] and mod(a,1)>0 [aperiodic]
...
%AvalancheSeries3 is the activation status of all neural units in lattice3
%sequence(:,1) is the total no. of units activated in lattice 
%sequence(:,2) is the total no. of units activated in lattice 2
%sequence(:,3) is the total no. of units activated in lattice 3
%Connectivity between the three lattices can be modified, follow instructions on the script
...
matrix3=zeros(sqrt(ROI3),sqrt(ROI3));
for a=1:sqrt(ROI3)
    for b=1:sqrt(ROI3)
...
xmax=1*((DistanceMatrix3(a,b)^exponent)*connect*adjust);        
p=6;
matrix3(a,b)=xmin+((xmax-xmin)*(sum(rand(1,p),2)/p));  
...
%matrix3
for a=1:sqrt(ROI3)
if AvalancheSeries3(a,time)==1;

for b=1:sqrt(ROI3)
   x=rand;
   if x<abs(matrix3(a,b));
       if matrix3(a,b)>0;
   AvalanchePlaceholder5(b,a)=1;
       else
           AvalanchePlaceholder5(b,a)=-1;
       end
   else
       AvalanchePlaceholder5(b,a)=0;
   end
end
    
else
    AvalanchePlaceholder5(:,a)=0;
...
lag=1;
     ...
     AvalancheSeries2(a,time+lag)=1; 
       ...
       if time>3
       if mod(a,1)==0;
      if  AvalanchePlaceholder4(a,time-3)>0 ||AvalanchePlaceholder4(a,time-2)>0 || AvalanchePlaceholder4(a,time-1)>0;
     AvalancheSeries2(a,time+lag)=1; 
      else
          AvalancheSeries2(a,time+lag)=0;
      end
  ...
  %matrix3
for b=1:sqrt(ROI3)
    AvalanchePlaceholder6(b,time)=sum(AvalanchePlaceholder5(b,:));
    if AvalancheSeries3(b,time)==1;
        AvalanchePlaceholder6(b,time)=0;       
    else
    end
    if  b>0
 repol=7;
     ...
     if time>repol;
           if sum(AvalancheSeries3(b,time-repol:time))>0
               AvalanchePlaceholder6(b,time)=0;
           else
           end
     end
       if time<repol+1
         if sum(AvalancheSeries3(b,1:time))>0
             AvalanchePlaceholder6(b,time)=0;
         else
         end
     end  
  ...
  for a=1:sqrt(ROI3)
lag=1;
      if AvalanchePlaceholder6(a,time)>0
     AvalancheSeries3(a,time+lag)=1; 
      else 
          AvalancheSeries3(a,time+lag)=0;
      end
      
       if time>3
       if mod(a,1)==0;
      if  AvalanchePlaceholder6(a,time-3)>0 ||AvalanchePlaceholder6(a,time-2)>0 || AvalanchePlaceholder6(a,time-1)>0;
     AvalancheSeries3(a,time+lag)=1; 
      else
          AvalancheSeries3(a,time+lag)=0;
      end
      else
    if  AvalanchePlaceholder6(a,time-1) >0;
     AvalancheSeries3(a,time+lag)=1; 
    end
       end
   else
  ...
  for a=1:sqrt(ROI3)     
   ...
   if time>3
      ...
      if AvalanchePlaceholder2(a,time-3)>0; 
          x=rand;
          if x<1; %Strength of lattice1->lattice2 connection
       AvalancheSeries2(a,time+lag)=0;
          else
          end
      else    
   ...
   if time>3
      ...
      if AvalanchePlaceholder4(a,time-3)>0;
          x=rand;
          if x<0; %Strength of lattice2->lattice1 connection
       AvalancheSeries(a,time+lag)=1;
          else
          end
      else    
   ...
   if time>3
      ...
      if AvalanchePlaceholder2(a,time-3)>0 
          x=rand;
          if x<0; %Strength of lattice1->lattice3 connection
       AvalancheSeries3(a,time+lag)=1;
          else
          end
      else    
   ...
   if time>3
        if mod(a,1)==0;
      if AvalanchePlaceholder6(a,time-3)>0;
          x=rand;
          if x<0; %Strength of lattice3->lattice1 connection
       AvalancheSeries(a,time+lag)=1;
          else
          end
      else    
   ...
   if time>3
      ...
      if AvalanchePlaceholder4(a,time-3)>0;
          x=rand;
          if x<1; %Strength of lattice2->lattice3 connection
       AvalancheSeries3(a,time+lag)=1;
          else
          end
      else    
   ...
   if time>3
      ...
      if AvalanchePlaceholder6(a,time-3)>0;
          x=rand;
          if x<0; %Strength of lattice3->lattice2 connection
       AvalancheSeries2(a,time+lag)=1;
          else
          end
      else    
...
sequence(time,3)=sum(AvalancheSeries3(1:sqrt(ROI3),time));;
end
