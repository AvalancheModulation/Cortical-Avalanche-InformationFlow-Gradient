function [Hal,He,Ht,pval95,HurstValues] = hurst(x,d,fontsize);
...
ERSal = sqrt(0.5*pi.*d);
 ...
 A(:,1)=log10(d);
    A(:,2)=log10( RSe - ERS + ERSal );
    index=isnan(A(:,2));
    A(index,:)=[];
Pal = polyfit(A(:,1),A(:,2),1);
Hal = Pal(1);
...
A(:,1)=log10(d);
    ...
    index=isnan(A(:,2));
    A(index,:)=[];
Pe = polyfit(A(:,1),A(:,2),1);
He = Pe(1);
P = polyfit(log10(d),log10(ERS),1);
Ht = P(1);
HurstValues(:,1)=log10(d);
HurstValues(:,2)=log10(RSe);
HurstValues(:,3)=log10(ERS);
